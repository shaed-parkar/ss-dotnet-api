...
public class LoggingDataExtractor : ILoggingDataExtractor
{
    public Dictionary<string, object> ConvertToDictionary(object? input, string path = null!, int depth = 0)
    {
        ...
        foreach (var property in type.GetProperties())
        {
            ...
            if (IsCustomType(property.PropertyType))
            {
                ...
                foreach (var kvp in propertyValues) result.Add(kvp.Key, kvp.Value);
            }
            else if (property.PropertyType != typeof(string) && property.PropertyType.GetInterfaces()
                         .Any(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IEnumerable<>)))
            {
                // Could handle IEnumerables here
            }
            else
            {
    ...
    private string GetPath(string path, string property)
    {
        return $"{path}{(string.IsNullOrEmpty(path) ? string.Empty : ".")}{property}";
    }
